...
import { useTronTransaction } from "~/hooks/useTronTransaction";
...
const _QuoteActionTron = (props: Omit<ButtonProps, "children">) => {
  const [isClicked, setIsClicked] = useState(false);
  const [buildError, setBuildError] = useState<Error | null>(null);

  const buildAndSendTransaction = useTronTransaction();
  ...
  const handleQuoteClick = useCallback(async () => {
    ...
    try {
      ...
      matchQuoteByType(quote, {
        swap: async () => {
          throw new Error("Swap quotes are not supported on TRON chains yet");
        },
    ...
    if (!connectedWallets[Chain.TRON]) {
      return (
        <Button
          {...props}
          onClick={(e) => {
            openAppKit({ view: "Connect", namespace: "tron" });
            props.onClick?.(e);
          }}
        >
          Connect wallet
...
export const QuoteActionTron = withTronWalletGuard(_QuoteActionTron);
